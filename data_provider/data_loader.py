...
class Dataset_Custom_MultiVariates(Dataset):
...
class Dataset_ETT_hour_multivariates(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h',
...
class Dataset_ETT_minute_multivariates(Dataset):
    def __init__(self, root_path, flag='train', size=None,
    ...
    def __read_data__(self):
        ...
        #     self.data_x = np.concatenate((self.data_x[::4], self.data_x, self.data_x[::48]), axis=0)
        #     self.data_y = np.concatenate((self.data_y[::4], self.data_y, self.data_y[::48]), axis=0)
        self.data_stamp = data_stamp
    ...
    def __len__(self):
        # if self.set_type == 0:
        #     return int((len(self.data_x) - self.seq_len - self.pred_len + 1)*self.enc_in*(self.percent / 100))  # 这里输入的时候就把展成单变量了
        # else:
        #return (len(self.data_x) - self.seq_len - self.pred_len + 1)*self.enc_in
        return len(self.data_x) - self.seq_len - self.pred_len + 1 # 这里输入的时候就把展成单变量了
